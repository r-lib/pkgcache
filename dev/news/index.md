...
- pkgcache now detects the `macos.binary.arm64` package type that
  R-devel (4.7.0) uses on macOS arm64, and looks for binaries in
...
- Binary packages from a PPM `manylinux` repository now have the correct
  ...
  now reports the correct glibc version of `manylinux` repositories.
...
- [`current_r_platform_data()`](https://r-lib.github.io/pkgcache/dev/reference/current_r_platform.md)
  gained a `pkg_type` column, for custom binary package types.

- [`current_r_platform()`](https://r-lib.github.io/pkgcache/dev/reference/current_r_platform.md)
  now handles setting `PKG_CURRENT_PLATFORM` to a non-Linux platform
  correctly.
