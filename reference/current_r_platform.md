...
`current_r_platform_data()` returns a data frame with character scalar
columns:
...
`default_platforms()` returns a character vector of the default
platforms.
...
[`cranlike_metadata_cache$new()`](https://r-lib.github.io/pkgcache/reference/cranlike_metadata_cache.md),
...
- `"macos"` for macOS binaries that are appropriate for the R versions
  pkgcache is working with. Packages for incompatible CPU architectures
  are dropped (defaulting to the CPU of the current macOS machine and
  x86_64 on non-macOS systems). The macOS Darwin version is selected
  based on the CRAN macOS binaries. E.g. on R 3.5.0 macOS binaries are
  built for macOS El Capitan.
