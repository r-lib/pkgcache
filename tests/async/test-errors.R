...
test_that("error propagates", {
  do <- async(function() {
    called <- FALSE
    dx <- delay(1 / 10000)$then(function(.) .)$then(function() {
      stop("ohno!")
...
test_that("handled error is not an error any more", {
  do <- async(function() {
    delay(1 / 10000)$then(function(x) stop("ohno!"))$catch(error = function(x) {
      "OK"
    })$then(function(.) expect_equal(., "OK"))$catch(error = function() {
      stop("not called")
    })
  })
  synchronise(do())
...
test_that("catch", {
  do <- async(function() {
    dx <- delay(1 / 1000)$then(function(.) .)$then(function() {
      stop("ooops")
    })$then(function() "not this one")$catch(error = function(.) {
      "nothing to see here"
    })
  })
  expect_equal(
...
test_that("catch handers", {
  ...
  do <- async(function() {
    async_constant(42)$then(function() {
      ...
      stop(err)
    })$catch(special = function(e) spec <<- e)$catch(foobar = function(e) {
      foobar1 <<- e
