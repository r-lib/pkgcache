...
- `pkgcache_low_speed_limit` and `pkgcache_low_speed_time` are used for a
  ...
  curl options.
- `pkgcache_http_version` selects the HTTP version to use for HTTP requests. It
  corresponds to the [`HTTP_VERSION` libcurl option](https://curl.se/libcurl/c/CURLOPT_HTTP_VERSION.html),
  so e.g. `2` forces HTTP/1.1 and `0` lets libcurl choose. It defaults to
...
- You can use the `PKG_CURRENT_PLATFORM` environment variable to set the
  ...
  Alternatively, you can use the `pkg.current_platform` option, which takes
